...
//转发到客服
type TransferKf struct {
	message.MsgCommon
	TransInfo struct {
		KfAccount     string `xml:"KfAccount,omitempty"  json:"KfAccount,omitempty"`
	} `xml:"TransInfo,omitempty"  json:"TransInfo,omitempty"`
}

func NewTransferKf(kfAccount string) (transferKf *TransferKf) {
	transferKf = new(TransferKf)
	if kfAccount !=""{
		transferKf.TransInfo.KfAccount = kfAccount
	}
	return
}